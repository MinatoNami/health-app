...
- **Auth:** a long-lived bearer token in the **Keychain** (`kSecAttrAccessibleAfterFirstUnlock`, so background uploads work on a locked device) is proportionate for a single-user self-hosted setup. OAuth is overkill. If you want more: pin your server's certificate, or use mTLS with a client cert in the Keychain.
- **TLS only.** No exceptions, no ATS exemptions. This is health data leaving a device.
